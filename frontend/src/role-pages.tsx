    ...
    <main className="admin-notification-page">
      <div className="booking-api-card">
        ...
        <form onSubmit={submit} className="booking-api-form">
          <label>Dịch vụ / gói khám<select required value={form.serviceId} onChange={(event) => update('serviceId', event.target.value)}><option value="">Chọn dịch vụ</option>{services.map((service) => { const price = Number(service.price ?? service.dongia ?? 0); return <option key={service.id ?? service.dichvuid} value={service.id ?? service.dichvuid}>{service.name || service.tendichvu}{price > 0 ? ` · ${price.toLocaleString('vi-VN')} đ` : ''}</option>; })}</select></label>
