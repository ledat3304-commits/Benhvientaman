...
ALTER TABLE benhnhan ADD COLUMN IF NOT EXISTS Email varchar(100);
...
CREATE TABLE thongbao (
    ...
    CONSTRAINT fk_thongbao_user FOREIGN KEY (UserID) REFERENCES nguoidung(UserID) ON DELETE CASCADE,
    CONSTRAINT fk_thongbao_lichkham FOREIGN KEY (LichKhamID) REFERENCES lichkham(LichKhamID) ON DELETE SET NULL
);
...
CREATE INDEX idx_thongbao_user_unread ON thongbao(UserID, DaDoc, NgayTao DESC);
CREATE INDEX idx_thongbao_lichkham ON thongbao(LichKhamID);
...
BEGIN
    ...
    SELECT admin.UserID, 'NEW_USER', 'Tài khoản mới đăng ký',
...
BEGIN
    ...
    SELECT admin.UserID, NEW.LichKhamID, 'NEW_APPOINTMENT', 'Có lịch khám mới',
           format('Người bệnh: %s | SĐT: %s | Email: %s | Bác sĩ: %s | Thời gian: %s | Lý do: %s',
                  patient_name, patient_phone, patient_email, doctor_name,
...
FOR EACH ROW EXECUTE FUNCTION fn_notify_admin_new_user();

CREATE TRIGGER trg_notify_admin_new_appointment
AFTER INSERT ON lichkham
FOR EACH ROW EXECUTE FUNCTION fn_notify_admin_new_appointment();
